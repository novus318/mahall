...
data = data
  .map((item:any) => ({
    ...
    members: item.members.filter((member:any) => {
      // Ensure member properties exist before calling .includes()
      const memberName = member.name ? member.name.toLowerCase().replace(/\s+/g, '') : '';
  ...
  .filter((item) => {
    ...
    return (
      houseName.includes(searchTerm) || // Search by house name
      houseNumber.includes(searchTerm) || // Search by house number
